...
from src.config.device_models import DEVICE_MODELS, get_all_models, get_models_by_brand, \
    get_tac_by_brand_and_model, get_tac_by_full_model
from src.config.carrier_info import CARRIER_INFO, get_mcc_by_region, get_mnc_by_carrier, \
    get_carriers_by_region, get_phone_prefix_by_region
from src.utils.checksum_utils import calculate_luhn_check_digit
...
class DeviceInfoGenerator:
    ...
    def _generate_imei(self, model: str) -> str:
        ...
        check_digit = calculate_luhn_check_digit(imei_without_check_digit)
    ...
    def _generate_accelerometer_data(self) -> dict:
        """生成加速度传感器数据"""
        return {
    ...
    def _generate_gyroscope_data(self) -> dict:
        """生成陀螺仪传感器数据"""
        return {
